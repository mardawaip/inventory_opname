				...
				<div class="container-fluid">
					...
					<div class="panel panel-headline">
						<div class="panel-heading">
							<h3 class="panel-title">Blanko KTP</h3>
						</div>
						<div class="panel-body">
							<div class="row">
								<div class="mb-8">
									<button id="btn-add" class="btn btn-success btn-xs edit-modal" data-toggle="modal" data-target="#modal-ktp"><i class="fa fa-plus"></i> Tambah</button>
							...
							<div class="row">
									...
									<table class="table table-striped table-bordered table-hover" id="table-ktp">
									<thead>
										<tr>
											<th>Tanggal</th>
											<th>Terima</th>
...
<!--=======modal edit ktp=========-->
<div class="modal fade" id="modal-ktp" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="CenterTitle">Form Blanko KTP</h5>
...
<script type = "text/javascript">
	$(document).ready( function () {
		var table = $('#table-ktp').DataTable({ 
			"processing": true,
			"serverSide": true,
			"order" : [],
			"ajax": {
				"url": '<?php echo base_url('admin/datatable_blank_ktp'); ?>',
				"type": "POST"
			},
					select: {
				"style" :    'os',
	...
	$('#btn-delete').click(function(){
		...
		if (!data) {
			...
			var r = confirm("Hapus KTP untuk tanggal "+data.tanggal+" ?");
			if (r == true) {
				$.ajax({
					type : "POST",
					url : "<?php echo base_url('admin/hapus_ktp'); ?>",
	...
	$('#btn-save').click(function(){
		...
		$.ajax({
			type: "POST",
			url: "/inventory/admin/add_ktp",
			data: data,

			success: function (response) {
				if (response == "success") {
					alert('Data berhasil disimpan');
					window.location.replace("<?php echo base_url('admin'); ?>/blankoktp");
					$('#modal-ktp').modal('hidden');
				} else{
					alert(response);
				}
